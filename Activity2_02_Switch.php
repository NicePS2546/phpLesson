...
<body>
    <form action='Activity2_02_Switch.php' class="form-alignment" method='post'>
    ...
    if($_SERVER['REQUEST_METHOD'] == 'POST'){//ถ้า server มีการรับข้อมูลจากฝั่ง html ด้วย method post ให้ทำโค๊ดต่อไปนี้
        $Member = $_POST['member'];//เก็บค่า variable ของตัวแปลที่ชื่อ member ใน html
        $isMember = false;
        if($Member === "member"){
            $isMember = true;
        }else{
        ...
        class Customer{//ประกาศ Class Customer
         public $product_price;//ประกาศตัวแปลต่างๆ
    ...
    //ในกรณีที่จะเรียกใช้ จะนำตัวแปล $productDetail['ชื่อของkeyที่ต้องการ'] ซึ่งแต่ละ key จะเก็บ value ที่เรากำหนดค่าไว้
